...
export default () => {
  ...
  self.addEventListener("message", event => {    
    ...
    if (operation === 'init'){
      ...
      console.log("worker | after setVonageMetadata");
      createMediaProcessor().then( () => {
        const msg = {callbackType: 'success', message: operation}
        console.log("worker | createMediaProcessor postmessage 1", msg);
        postMessage(JSON.stringify(msg));
      }).catch(e=> {
        console.log("worker | createMediaProcessor err 2", e);
        const msg = {callbackType: 'error', message: operation, error: getErrorMessage(e)}
        console.log("worker | createMediaProcessor postmessage 2", msg);
        postMessage(JSON.stringify(msg));
      })
  ...
  function createMediaProcessor() {
    return new Promise((resolve, reject) => {
      ...
      console.log("222");
      resolve();
      // console.log("worker | mediaProcessor", mediaProcessor);
      ...
      //   console.log("worker | setTransformers");
